...
def plot_errors_comparison(feature_name, n_hidden, file_number=1):
    ...
    # Plot train error with error bars
    ax1.errorbar(alphas_emp, train_errors_emp, yerr=train_std_emp/np.sqrt(n_iter), color='blue',
                fmt='o', alpha=0.7, label='Empirical', capsize=2, markersize=3)
    ax1.plot(alphas_theory, train_errors_theory, '-', label='Theory', alpha=0.7, color='darkblue')
    ...
    ax2.errorbar(alphas_emp, test_errors_emp, yerr=test_std_emp/np.sqrt(n_iter), color='blue',
                fmt='o', alpha=0.7, label='Empirical', capsize=2, markersize=3)
    ax2.plot(alphas_theory, test_errors_theory, '-', label='Theory', alpha=0.7, color='darkblue')
    ...
    ax3.errorbar(alphas_emp, class_errors_emp, yerr=class_std_emp/np.sqrt(n_iter), color='blue',
                fmt='o', alpha=0.7, label='Empirical', capsize=2, markersize=3)
    ax3.plot(alphas_theory, class_errors_theory, '-', label='Theory', alpha=0.7, color='darkblue')
    ax3.set_xlabel(r'$\alpha$')
    ax3.set_ylabel('Classification Error')
    ax3.set_ylim(0.24, 0.36)  # Set y-axis limits
    ax3.legend()
    ...
    plt.savefig(os.path.join(save_dir, f'errors_comparison_{feature_name}_n_hidden_{n_hidden}.pdf'))
    plt.close()
