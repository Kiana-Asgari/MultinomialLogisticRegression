...
def run_and_log_mle(
    ...
    if type_3 == False and non_symmetric:
        ...
        base_filename = (
            f"mle_data_k{k}_k0{k_0}_lambda{lambda_reg}_d{d}_ntrials{n_trials}_k3.json"
        )
    elif type_3 != False:
            ...
            with warnings.catch_warnings(record=True) as w:
                ...
                try:
                    emp_results = fit_mle_skitlearn(
                        alpha=alpha, k=k, R_00=R_00, n_trials=n_trials, d=d
                    )
                    norms, test_errors, train_errors = (
                        emp_results["norms"],
                        emp_results["test_errors"],
                        emp_results["train_errors"],
                    )
                    ...
                    print(
                        ...
                        np.std(misclassification_test_errors),
                    )
                    # Check if overflow warning was raised
                    if any(
                        issubclass(warn.category, RuntimeWarning)
                        and "overflow" in str(warn.message)
                        for warn in w
                    ):
                    ...
                    if diverged:
                        ...
                        print(
                            f"Diverged due to large norm (avg_norm={avg_norm:.2f}) for alpha={alpha}"
                        )
                    else:
                        results[R_00_str][alpha_str] = {
                            "norm": norms.tolist(),
                            ...
                            "shapes": {
                                ...
                                "misclassification_test_errors": list(
                                    misclassification_test_errors.shape
                                ),
                            },
                            "diverged": False,
                        }
