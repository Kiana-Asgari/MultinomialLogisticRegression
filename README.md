...
$$S_{t+1} = \frac{1}{\alpha} \left(I - \mathbb{E}\left[\left(I + S \cdot \text{Jp}(\text{prox}(g + yS; S))\right)^{-1}\right] + 2\lambda_{\text{reg}}S_t\right)^{-1} \cdot S_t$$
...
$$\text{schur}_{t+1} = \alpha \cdot S_{t+1} \cdot \mathbb{E}\left[(p(v)-y) \cdot (p(v)-y)^T\right] \cdot S_{t+1}$$
