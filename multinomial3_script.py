...
if __name__ == "__main__":
    ...
    for _ in range(10):
        ...
        refine_logged_regulairzed_fp(k_0=k_0, k=k,type_3=type_3, max_iter=1,
                                    tol=1e-5, integral_mesh_size=13, integral_size=4.4,
                                    lambda_reg_min=0.02, lambda_reg_max=0.03,
                                    dtype=torch.float32, modified_alpha=alpha)
        refine_logged_fp_tests_regularized(k_0=k_0, k=k,type_3=type_3, R_00=R00, 
                                            lambda_reg_min=0.01, lambda_reg_max=0.05,
                                            metric_name='misclassification_test_errors',
                                           modified_alpha=alpha)
        plot_regularized_error(k=k, k_0=k_0, R_00=R00, type_3=type_3, d=300, n_trials=100, lambda_reg_max=0.06, lambda_reg_min=0.0)
    sys.exit()

    type_3 = 'three_classes_close'
    R00 = get_R_00(k, type_3)
    run_and_log_fp(k_0=k_0, k=k, type_3=type_3, tol=5*1e-2, use_lambda_reg=0, alphas=[4.75],
                                        max_iter=10, integral_size=4.4, integral_mesh_size=13, dtype=torch.float32)
    run_and_log_fp_tests(k_0=k_0, k=k, type_3=type_3, metric_name='train_error', alpha_min=4.75, alpha_max=5)
    sys.exit()
    for _ in range(10): #running 4.80 in three classes close
        refine_logged_fp(k_0=k_0, k=k, type_3=type_3, tol=5*1e-2, use_lambda_reg=0,
                                        max_iter=1, integral_size=4.4, integral_mesh_size=13, 
                                        alpha_max=4.91, alpha_min=4.75, dtype=torch.float32)
        refine_logged_fp_tests(k_0=k_0, k=k, type_3=type_3, metric_name='train_error', alpha_min=4.75, alpha_max=5)


        plot_errors_vs_alpha(k=k, k_0=k_0, alpha_min=4, alpha_max=6, d=300, n_trials=100 )
